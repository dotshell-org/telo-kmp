...
import eu.dotshell.telo.generic.data.repository.TransportRepository
...
class TransportViewModel(private val context: PlatformContext) : ViewModel(), TransportViewModelInterface {
    ...
    private val trafficAlertsRepository = TrafficAlertsRepository(
        transportApi,
        eu.dotshell.telo.platform.Settings(context, "traffic_alerts_cache"),
        eu.dotshell.telo.generic.data.offline.OfflineRepository(context)
    )
    val userStopAlertsRepository by lazy {
        UserStopAlertsRepository(
            transportApi as? eu.dotshell.telo.generic.data.network.UserStopAlertsApi
        )
    }
    ...
    private val holidayDetector by lazy {
        ...
        val holidayFileName = (config as? eu.dotshell.telo.generic.data.config.AppTransportConfig)?.schoolHolidaysFile ?: "holidays.json"
    ...
    private val transportCache by lazy { eu.dotshell.telo.generic.data.cache.TransportCacheImpl(context) }
    private val offlineRepository by lazy { eu.dotshell.telo.generic.data.offline.OfflineRepository(context) }
    ...
    private fun parseLineMentionsFromText(raw: String): Set<String> {
        return eu.dotshell.telo.generic.ui.viewmodel.parseLineMentionsFromText(raw, lineRules)
    }

    private fun parseAlertTokens(raw: String): Set<String> {
        return eu.dotshell.telo.generic.ui.viewmodel.parseAlertTokens(raw, lineRules)
    }

    private fun parseTimeToMinutes(rawTime: String): Int? {
        return eu.dotshell.telo.generic.ui.viewmodel.parseTimeToMinutes(rawTime)
    }

    private fun pickNextDeparture(schedules: List<String>, currentMinutes: Int): String? {
        return eu.dotshell.telo.generic.ui.viewmodel.pickNextDeparture(schedules, currentMinutes)
    }

    override fun parseLineCodesFromDesserte(desserte: String): List<String> {
        return eu.dotshell.telo.generic.ui.viewmodel.parseLineCodesFromDesserte(desserte)
    }

    private fun normalizeStopName(stopName: String): String {
        return eu.dotshell.telo.generic.ui.viewmodel.normalizeStopName(stopName)
    }
    ...
    private fun findStopByCoordinates(
        ...
        val closestStop = eu.dotshell.telo.generic.ui.viewmodel.findStopByCoordinates(stops, targetLat, targetLon)
        if (closestStop != null) {
