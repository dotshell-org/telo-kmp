...
import eu.dotshell.telo.generic.data.telemetry.TelemetryUploader
...
actual class BackgroundScheduler actual constructor(context: PlatformContext) {
    ...
    actual fun scheduleTelemetryUpload(delaySeconds: Long) {
        val request = BGAppRefreshTaskRequest("eu.dotshell.telo.telemetryUpload")
    ...
    actual fun cancelTelemetryUpload() {
        BGTaskScheduler.sharedScheduler.cancelTaskRequestWithIdentifier("eu.dotshell.telo.telemetryUpload")
        Log.i(TAG, "cancelTelemetryUpload() called")
    }

    actual fun ensureTrafficAlertsScheduled() {
        val request = BGAppRefreshTaskRequest("eu.dotshell.telo.trafficAlerts")
